...
export async function runHousekeeping(env: Env): Promise<void> {
	...
	const accountRows = await DB.prepare('SELECT id, plan FROM accounts').all<{
		id: string;
		plan: string;
	}>();
	...
	for (const { id: accountId, plan } of accountRows.results ?? []) {
		try {
			...
			await DB.prepare('DELETE FROM zone_snapshots WHERE account_id = ? AND timestamp < ?')
			...
			console.log(
				`[Housekeeping] Account ${accountId}: cleaned up data older than ${retentionDays} days`,
			);
		} catch (err) {
